<%@ Page Title="" Language="C#" MasterPageFile="~/Staff.Master" AutoEventWireup="true" CodeBehind="StaffAcademicPage.aspx.cs" Inherits="SchoolProject.StaffAcademicPage" %>
              ...
              <fieldset>
                <div class="row">
                 ...
                 <div style="overflow:scroll">
                     ...
                     <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConflictDetection="CompareAllValues" ConnectionString="<%$ ConnectionStrings:SMSConnectionString %>" DeleteCommand="DELETE FROM [Academic_info] WHERE [Id] = @original_Id AND (([Date] = @original_Date) OR ([Date] IS NULL AND @original_Date IS NULL)) AND (([ClassName] = @original_ClassName) OR ([ClassName] IS NULL AND @original_ClassName IS NULL)) AND (([YearofPassing] = @original_YearofPassing) OR ([YearofPassing] IS NULL AND @original_YearofPassing IS NULL)) AND (([Result] = @original_Result) OR ([Result] IS NULL AND @original_Result IS NULL)) AND (([Percentage] = @original_Percentage) OR ([Percentage] IS NULL AND @original_Percentage IS NULL)) AND (([Board] = @original_Board) OR ([Board] IS NULL AND @original_Board IS NULL))" InsertCommand="INSERT INTO [Academic_info] ([Id], [Date], [ClassName], [YearofPassing], [Result], [Percentage], [Board]) VALUES (@Id, @Date, @ClassName, @YearofPassing, @Result, @Percentage, @Board)" OldValuesParameterFormatString="original_{0}" SelectCommand="SELECT * FROM [Academic_info]" UpdateCommand="UPDATE [Academic_info] SET [Date] = @Date, [ClassName] = @ClassName, [YearofPassing] = @YearofPassing, [Result] = @Result, [Percentage] = @Percentage, [Board] = @Board WHERE [Id] = @original_Id AND (([Date] = @original_Date) OR ([Date] IS NULL AND @original_Date IS NULL)) AND (([ClassName] = @original_ClassName) OR ([ClassName] IS NULL AND @original_ClassName IS NULL)) AND (([YearofPassing] = @original_YearofPassing) OR ([YearofPassing] IS NULL AND @original_YearofPassing IS NULL)) AND (([Result] = @original_Result) OR ([Result] IS NULL AND @original_Result IS NULL)) AND (([Percentage] = @original_Percentage) OR ([Percentage] IS NULL AND @original_Percentage IS NULL)) AND (([Board] = @original_Board) OR ([Board] IS NULL AND @original_Board IS NULL))">
                         <DeleteParameters>
                             ...
                             <asp:Parameter Name="original_Board" Type="String" />
                         </DeleteParameters>
                         <InsertParameters>
